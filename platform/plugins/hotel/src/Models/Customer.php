    ...
    CanResetPasswordContract
{
    ...
    protected function avatarUrl(): Attribute
    {
        return Attribute::make(
            get: function () {
                ...
                try {
                    // Check if GD FreeType is available before attempting
                    if (!function_exists('imageftbbox')) {
                        throw new Exception('GD FreeType not supported');
                    }
                    return (new Avatar())->create(Str::ucfirst($this->name))->toBase64();
                } catch (Exception $e) {
                    // Log the error if needed: \Log::warning('Avatar generation failed: ' . $e->getMessage());
                    return RvMedia::getDefaultImage();
                }
            }
        );
    ...
    {
        static::deleting(function (Customer $account): void {
